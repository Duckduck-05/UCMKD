...
def eval_overlap_tag(loader, device, args):
    ...
    tea = TeaViT(feat_dim=tea_model.feature_dim, num_classes=6).to(device) if _tea_is_vit else Tea().to(device)
    stu = StuViT(feat_dim=net.feature_dim, num_classes=6).to(device) if _stu_is_vit else Stu().to(device)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data-root', type=str, default='',
                        help='root directory of the CREMA-D dataset')
    ...
    if args.cmkd:
        ...
        for run in range(args.num_runs):
            set_random_seed(run)
            print(f'Seed {run}')
            run_name = (f"data:cremad, {args.method_type}, seed:{run}, "
