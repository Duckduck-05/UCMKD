...
def eval_overlap_tag(loader, device, args):
    ...
    _image_is_vit = args.image_arch in ('vit_b_16', 'vit_l_16', 'vit_s_16')
    ...
    tea = TeaViT(feat_dim=tea_model.feature_dim, num_classes=309).to(device) if _tea_is_vit else Tea().to(device)
    stu = StuViT(feat_dim=net.feature_dim, num_classes=309).to(device) if _stu_is_vit else Stu().to(device)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--method_type', type=str, default='ce',
                        choices=['bilevel', 'ce', 'sumall', 'vanillaKD', 'feadistill', 'reviewkd', 'norm'])
    parser.add_argument('--metric', type=str, default='cosine',
                        choices=['l1', 'l2', 'cosine', 'chordal'])
    ...
    if args.cmkd:
        ...
        for run in range(args.num_runs):
            set_random_seed(run)
            print(f'Seed {run}')
            run_name = (f"data:vggsound, {args.method_type}, seed:{run}, "
