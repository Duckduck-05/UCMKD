...
def gen_data(data_dir, batch_size, num_workers, args):
    ...
    val_dataset   = VGGSound(args, mode='val')
...
def train_network_distill_c2kd(stu_type, tea_model, epochs, loader, net, device, optimizer, args, stu, tea):
    save_model = True
    ...
    for epoch in range(epochs):
        ...
        for i, data in enumerate(loader['train']):
            ...
            loss.backward()
            optimizer.step()
            _ = adjust_lr(iter=epoch, optimizer=optimizer)
        ...
        print(
            f"Train Loss: {train_loss / len(loader['train']):.3f} | GT Loss {loss1 / len(loader['train']):.3f} | PL Loss {loss2 / len(loader['train']):.3f} | PL_pseu_label Loss {loss_s2t / len(loader['train']):.3f} | PL_output Loss {loss_t2s / len(loader['train']):.3f}| cosine Loss {loss_ang / len(loader['train']):.3f}")
...
def pre_train_models(stu_type, tea_type, loader, epochs, learning_rate, device, args, save_model=False):
    ...
    for epoch in range(epochs):
        ...
        if epoch > 80:
            ...
            print(f"Best Teacher Train|Test acc: {val_best_acc:.2f}|{test_best_acc:.2f}")
