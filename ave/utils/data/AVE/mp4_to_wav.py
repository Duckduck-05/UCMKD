...
all_videos = os.environ.get('AVE_ANNOTATIONS', 'Annotations.txt')
all_audio_dir = os.environ.get('AVE_AUDIO_DIR', 'Audios')
...
for i, item in enumerate(files[1:]):
    if i % 500 == 0:
        ...
        print('*******************************************')
    item = item.split('&')
    mp4_filename = os.path.join(os.environ.get('AVE_VIDEO_DIR', 'AVE'), item[1] + '.mp4')
